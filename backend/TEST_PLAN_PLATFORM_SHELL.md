...
RegistryService feature catalog exposes only plan-assignable features for the plan editor. Covered at unit level.
RegistryService permission catalog exposes only platform-control permissions for admin-role pickers. Covered at unit level.
RegistryService permission catalog exposes only explicit B2B-delegatable actions for B2B pickers. Covered at unit level.
```
...
QuotaExceededException -> 402: an account has exhausted the effective plan or override quota.
```

This contract is active for API-surface token separation, client workspace ownership checks, member, role, and invitation scope validation, B2B collaboration lifecycle enforcement, context resolution, permission-surface validation, quota exhaustion, and invalid subscription override parsing. Inactive member access remains `401` because an existing access token no longer yields a usable workspace actor context. `GlobalExceptionHandlerTest` now pins representative `ApiError` body shapes so frontend code can rely on status, error, message, timestamp, and quota details.
