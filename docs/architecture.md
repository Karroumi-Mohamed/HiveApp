...
Runtime interception is enabled at the platform namespace root: `com.hiveapp.platform` is annotated with `@PermissionNode(key = "platform", guard = Guard.ON)`. This root is required for Permissionizer's Spring AOP interceptor to enforce the feature and action annotations beneath it. Permission denial is an authorization outcome and is translated to HTTP `403`, not an unhandled server error.
